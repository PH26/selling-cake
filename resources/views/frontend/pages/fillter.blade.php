                        ...
                        <aside class="ps-widget ps-widget--sidebar ps-widget--category">
                                ...
                                <div class="ps-widget__content">
                                    <ul class="ps-list--circle">
                                        <li class="current">
                                        <a href="allproduct"><span  id="0" class="common_selector circle"></span>All category </a></li>
                                        @foreach($categories as $category)
                                        <li><a href="category/{{$category->id}}"><span id="{{$category->id}}" class="common_selector circle "></span>{{$category->name}}</a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </aside>
                            <aside class="ps-widget ps-widget--sidebar ps-widget--filter">
                                ...
                                <div class="ps-widget__content">
                                <form action="" method='GET' id=fillter>
                                            <input type="number" name="minimum_price" id='minimum_price' step="{!!number_format(10000,0,',','.')!!}" max="{!!number_format(300000,0,',','.')!!}" style="width: 80px; text-align:center;" value='{{$minPrice}}'/>

                                            <input type="number" name="maximum_price" id='maximum_price' step="{!!number_format(10000,0,',','.')!!}" max="{!!number_format(300000,0,',','.')!!}" style="width: 80px; text-align:center;" value='{{$maxPrice}}'/>
 ...
 <script type="text/javascript">
    $(document).ready(function () {
        ...
        for(var i = 0; i < span_select.length; i++){
            ...
            var id = current.split("/")[4];
            if (id == category_id){
                $("#" + category_id).parent().parent().attr('class',' current' );
                $("#fillter").attr('action', 'fillter/'+id );
            } else {
